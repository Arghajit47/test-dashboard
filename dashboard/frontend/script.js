...
function populateDeviceFilter(data) {
  ...
  deviceTypes.forEach((devices) => {
    const option = document.createElement("option");
    option.value = devices;
    option.textContent = devices;
...
function updateHorizontalBarChart(data) {
  ...
  const barData = testNames.map(
    (test) =>
      latestTests[test][latestTests[test].length - 1].performance_metrics
  );
  const tooltipData = testNames.map((test) =>
    latestTests[test].map((run) => run.performance_metrics)
  );
...
function updateSeoChart(data) {
  ...
  const testNames = Object.keys(latestTests);
  const barData = testNames.map(
    (test) => latestTests[test][latestTests[test].length - 1].seo_metrics
  );
  const tooltipData = testNames.map((test) =>
    latestTests[test].map((run) => run.seo_metrics)
  );
...
function updateAccessibilityChart(data) {
  ...
  const tooltipData = testNames.map((test) =>
    latestTests[test].map((run) => run.accessibility_metrics)
  );
...
function updateBestPracticeChart(data) {
  ...
  const barData = testNames.map(
    (test) =>
      latestTests[test][latestTests[test].length - 1].best_practice_metrics
  );
  const tooltipData = testNames.map((test) =>
    latestTests[test].map((run) => run.best_practice_metrics)
  );
...
function updateDashboard(data) {
  ...
  const deviceData = deviceLabels.map(
    (d) => devices[d].performance_metrics / devices[d].count
  );
